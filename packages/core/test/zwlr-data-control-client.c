// zwlr_data_control_v1 test client. Two roles:
//
...
//       control device, receive(), print "[zwlr-dc-client] received: <data>"
...
static struct zwlr_data_control_manager_v1* mgr = NULL;
static struct zwlr_data_control_device_v1* dev = NULL;
...
// --- source mode ---
static void srcSend(void* d, struct zwlr_data_control_source_v1* s, const char* mime, int32_t fd) {
...
static void srcCancelled(void* d, struct zwlr_data_control_source_v1* s) {
    (void)d; zwlr_data_control_source_v1_destroy(s);
}
static const struct zwlr_data_control_source_v1_listener srcListener = { srcSend, srcCancelled };

// --- receive mode ---
static struct zwlr_data_control_offer_v1* pending_offer = NULL;
static int offer_has_mime = 0;
static int got_selection = 0;
static int got_primary_selection = 0;

static void offOffer(void* d, struct zwlr_data_control_offer_v1* o, const char* mime) {
    (void)d; (void)o;
    if (g_mime && strcmp(mime, g_mime) == 0) offer_has_mime = 1;
}
static const struct zwlr_data_control_offer_v1_listener offListener = { offOffer };

static void devDataOffer(void* d, struct zwlr_data_control_device_v1* dd,
    ...
    pending_offer = offer;
    offer_has_mime = 0;
    zwlr_data_control_offer_v1_add_listener(offer, &offListener, NULL);
}
static void devSelection(void* d, struct zwlr_data_control_device_v1* dd,
                         struct zwlr_data_control_offer_v1* offer) {
    (void)d; (void)dd;
    got_selection = 1;
...
static void devFinished(void* d, struct zwlr_data_control_device_v1* dd) {
    (void)d; (void)dd;
    running = 0;
}
static void devPrimary(void* d, struct zwlr_data_control_device_v1* dd,
                       struct zwlr_data_control_offer_v1* offer) {
    (void)d; (void)dd;
    got_primary_selection = 1;
    if (offer == NULL && g_primary) pending_offer = NULL;
}
static const struct zwlr_data_control_device_v1_listener devListener = {
...
static void regGlobal(void* data, struct wl_registry* reg, uint32_t name,
    ...
    else if (strcmp(iface, "zwlr_data_control_manager_v1") == 0)
        mgr = wl_registry_bind(reg, name, &zwlr_data_control_manager_v1_interface, version < 2 ? version : 2);
}
...
int main(int argc, char** argv) {
    ...
    if (!display) { fprintf(stderr, "[zwlr-dc-client] connect failed\n"); return 1; }
    ...
    if (!seat || !mgr) {
        fprintf(stderr, "[zwlr-dc-client] missing globals (seat=%p mgr=%p)\n", (void*)seat, (void*)mgr);
        return 1;
    }
    dev = zwlr_data_control_manager_v1_get_data_device(mgr, seat);
    zwlr_data_control_device_v1_add_listener(dev, &devListener, NULL);
    ...
    wl_display_roundtrip(display);
    printf("[zwlr-dc-client] device ready\n");
    fflush(stdout);

    if (source_mode) {
        struct zwlr_data_control_source_v1* src =
            zwlr_data_control_manager_v1_create_data_source(mgr);
        zwlr_data_control_source_v1_add_listener(src, &srcListener, NULL);
        zwlr_data_control_source_v1_offer(src, g_mime);
        if (g_primary) zwlr_data_control_device_v1_set_primary_selection(dev, src);
        else zwlr_data_control_device_v1_set_selection(dev, src);
        wl_display_roundtrip(display);
        printf("[zwlr-dc-client] selection set mime=%s primary=%d\n", g_mime, g_primary);
        fflush(stdout);
    ...
    if (!*got || !pending_offer || !offer_has_mime) {
        fprintf(stderr,
            "[zwlr-dc-client] no matching offer (got=%d pending=%p mime_match=%d primary=%d)\n",
    ...
    zwlr_data_control_offer_v1_receive(pending_offer, g_mime, pipefd[1]);
    close(pipefd[1]);
    wl_display_flush(display);
    ...
    received[off] = '\0';
    close(pipefd[0]);
    printf("[zwlr-dc-client] received: %s\n", received);
    fflush(stdout);
    wl_display_disconnect(display);
    return off > 0 ? 0 : 1;
}
