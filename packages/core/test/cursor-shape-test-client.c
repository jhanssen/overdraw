// wp_cursor_shape_v1 test client: maps a toplevel, gets a
// wp_cursor_shape_device_v1 from the manager, and on pointer.enter calls
// set_shape(serial, default). The compositor's cursor slot should resolve
// the 'default' shape via the theme resolver (the built-in fallback
// arrow when no theme is installed) and render it at the pointer
// position.
//
// stdout signals:
//   "[client] mapped"
...
static struct wp_cursor_shape_manager_v1* shape_manager = NULL;
static struct wp_cursor_shape_device_v1* shape_device = NULL;
static int surface_configured = 0;
static uint32_t last_enter_serial = 0;
static int got_enter = 0;
static int set_shape_done = 0;
...
static void regGlobal(void* data, struct wl_registry* reg, uint32_t name,
    ...
    if (strcmp(iface, "wl_compositor") == 0)
        ...
        shape_manager = wl_registry_bind(reg, name, &wp_cursor_shape_manager_v1_interface,
                                         version < 2 ? version : 2);
    }
}
...
int main(int argc, char** argv) {
    ...
    if (!compositor || !shm || !wm_base || !seat || !shape_manager) {
        fprintf(stderr, "[client] missing globals (shape_manager=%p)\n", (void*)shape_manager);
        return 1;
    }
    ...
    xdg_toplevel_set_title(toplevel, "cs-test");
    ...
    // Get the per-pointer cursor shape device and request the 'default'
    // shape (enum value 1).
    shape_device = wp_cursor_shape_manager_v1_get_pointer(shape_manager, pointer);
    wp_cursor_shape_device_v1_set_shape(shape_device, last_enter_serial,
                                        WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_DEFAULT);
    wl_display_roundtrip(display);
    set_shape_done = 1;
    printf("[client] set_shape\n"); fflush(stdout);

    usleep(800 * 1000);

    wp_cursor_shape_device_v1_destroy(shape_device);
    ...
    close(fdTl);
    printf("[client] done\n");
    return (got_enter && set_shape_done) ? 0 : 1;
}
