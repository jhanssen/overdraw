...
test('appId regex: match floats, non-match is observe-only (undefined)', async () => {
  ...
  const hit = handler('window.preconfigure', payload({ appId: 'Netflix' }));
  ...
  assert.equal(handler('window.preconfigure', payload({ appId: 'firefox' })), undefined);
});

test('null appId never matches an appId regex', async () => {
  const { handler } = await load([{ match: { appId: '.*' }, float: true }]);
  assert.equal(handler('window.preconfigure', payload({ appId: null })), undefined);
});

test('title regex match', async () => {
  const { handler } = await load([{ match: { title: 'YouTube' }, float: true }]);
  const hit = handler('window.preconfigure', payload({ title: 'Cats - YouTube' }));
  assert.equal(hit.initialState.tiling, 'floating');
  assert.equal(handler('window.preconfigure', payload({ title: 'nope' })), undefined);
});
...
test('appId AND title: both present fields must match', async () => {
  ...
  assert.equal(handler('window.preconfigure', payload({ appId: 'mpv', title: 'Small' })), undefined);
  // both match -> yes
  const hit = handler('window.preconfigure', payload({ appId: 'mpv', title: 'Big Buck' }));
  assert.equal(hit.initialState.tiling, 'floating');
});
...
test('predicate match receives the window query incl. xwayland', async () => {
  ...
  assert.equal(handler('window.preconfigure', payload({ appId: 'steam', xwayland: false })), undefined);
  const hit = handler('window.preconfigure', payload({ appId: 'steam', xwayland: true }));
  assert.equal(hit.initialState.tiling, 'floating');
...
test('apply lambda gets read fields plus the mutable state proposal', async () => {
  ...
  const hit = handler('window.preconfigure', payload({ appId: 'foo', title: 'bar', surfaceId: 7 }));
  assert.equal(hit.initialState.tiling, 'floating');
...
test('apply lambda can mutate any proposal field (exclusive, visible, constraints)', async () => {
  ...
  const hit = handler('window.preconfigure', payload({ appId: 'media' }));
  assert.equal(hit.initialState.tiling, 'floating');
...
test('float:false explicitly tiles; lambda can tile via state', async () => {
  ...
  assert.equal(handler('window.preconfigure', p1).initialState.tiling, 'managed');
  const p2 = payload({ appId: 'y' });
  p2.initialState.tiling = 'floating';
  assert.equal(handler('window.preconfigure', p2).initialState.tiling, 'managed');
});
...
test('multiple matching rules: applied in order, last wins per axis', async () => {
  ...
  assert.equal(handler('window.preconfigure', payload({ appId: 'z' })).initialState.tiling, 'managed');
});
...
test('does not mutate the original initialState object', async () => {
  ...
  const hit = handler('window.preconfigure', p);
...
test('apply lambda that throws is caught (logged), does not abort interception', async () => {
  ...
  const hit = handler('window.preconfigure', payload({ appId: 'foo' }));
...
test('malformed payload is observe-only', async () => {
  const { handler } = await load([{ match: { appId: '.*' }, float: true }]);
  assert.equal(handler('window.preconfigure', null), undefined);
  assert.equal(handler('window.preconfigure', { surfaceId: 1 }), undefined); // no initialState
});
